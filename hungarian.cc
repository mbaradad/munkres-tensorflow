...
//TODO: extend to rectangular matrix
 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    for (int i = 0; i < costs_tensor.shape().dims(); ++i) {
      pred_shape.push_back(costs_tensor.shape().dim_size(i));
    }
    ...
    const int batch_size = pred_shape[0];
  ...
  vector<int> get_assignment(vector<float> match_cost, int num_pred) {
